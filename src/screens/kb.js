...
const KB_ENTRIES = [
  ...
  { id: 'kb4', title: 'Remplacement carte SIM & APN',     tags: ['gsm', 'modem', 'sim', 'apn'],   type: 'pdf', file: 'SIM Card Replacement and APN Setup Procedure.pdf' },
  { id: 'kb5', title: 'Tutoriel Vetimat Kalistrut',       tags: ['vetimat', 'kalistrut', 'tuto'], type: 'pdf', file: 'Tutoriel vetimat KALISTRUT.pdf' },
  { id: 'kb6', title: 'Tuto tresse',                      tags: ['tresse', 'epimat'],             type: 'pdf', file: 'tuto tresse.pdf' },
  ...
  if (!filtered.length) {
    list.innerHTML = `
      <div class="text-center py-10">
        <p class="text-sm text-slate-400 font-medium">Aucun résultat pour "${q}"</p>
      </div>`;
    return;
  }

  list.innerHTML = filtered.map(e => `
    <button class="tap-card w-full bg-white border border-slate-200 rounded-2xl p-4
