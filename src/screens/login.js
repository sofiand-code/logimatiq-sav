...
import { t, langToggleHTML, bindLangToggle } from '../i18n.js';
...
export function renderLogin(onDone) {
  ...
  container.innerHTML = `
    ...
    <div class="text-center mb-8">
      ...
      <h2 class="text-2xl font-black text-white">${t('Connexion')}</h2>
      <p class="text-sm mt-1" style="color:rgba(255,255,255,.5);">
        ${t('Identifiants fournis par Logimatiq')}
      </p>
    </div>
    ...
    <div class="space-y-3">
      <div>
        <label class="text-xs font-bold uppercase tracking-widest mb-1.5 block"
               style="color:rgba(255,255,255,.6)">${t('Identifiant')}</label>
        <input id="login-user" type="text" placeholder="${t('Votre login')}"
      ...
      <div>
        <label class="text-xs font-bold uppercase tracking-widest mb-1.5 block"
               style="color:rgba(255,255,255,.6)">${t('Mot de passe')}</label>
        <input id="login-pass" type="password" placeholder="${t('Votre mot de passe')}"
      ...
      <p class="text-center text-xs pt-2" style="color:rgba(255,255,255,.3);">
        ${t('Contactez Logimatiq pour obtenir vos identifiants')}<br/>
        07 45 28 44 83
      </p>
    </div>`;
