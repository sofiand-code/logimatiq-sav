  ...
  if (!history.length && !machines.length) {
    container.innerHTML = `
      <div class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p class="font-bold text-slate-500 text-sm">Aucune donnée disponible</p>
        <p class="text-xs text-slate-400 mt-1">Effectuez des diagnostics pour voir les statistiques</p>
      </div>`;
    return;
  }
  ...
  container.innerHTML = `
    ...
    <div class="grid grid-cols-3 gap-2 mb-5">
      <div class="bg-white rounded-2xl p-3 border border-slate-200 shadow-sm text-center">
        ...
        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">Diagnostics</div>
      </div>
      <div class="bg-white rounded-2xl p-3 border border-slate-200 shadow-sm text-center">
      ...
      <div class="bg-white rounded-2xl p-3 border border-slate-200 shadow-sm text-center">
        ...
        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">Machines</div>
      </div>
    </div>

    <!-- Résultats détaillés -->
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 mb-5">
      <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Répartition des résultats</p>
      ${outcomeBar('Résolu', resolved, total, '#10B981')}
      ${outcomeBar('SAV requis', sav, total, '#DC2626')}
      ${outcomeBar('Pièce à changer', part, total, '#F59E0B')}
    </div>
        ...
        ${topMachines.map((m, i) => `
          <div class="bg-white border border-slate-200 rounded-2xl p-3.5 shadow-sm flex items-center gap-3">
            ...
            <div class="text-right shrink-0">
              ...
              <div class="text-[10px] text-slate-400">${m.sav > 0 ? `${m.sav} SAV` : 'OK'}</div>
            </div>
          </div>`).join('')}
      </div>
    </div>` : ''}

    <!-- Sociétés les + actives (visible pour tech uniquement) -->
    ${(user?.role !== 'client' && topCompanies.length > 1) ? `
    <div class="mb-5">
      <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2.5">
        Sociétés les plus actives
      </h3>
      <div class="space-y-2">
